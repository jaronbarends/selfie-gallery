...
(function () {
  ...
  var Swiped = function (o) {
    var defaultOptions = {
      ...
      willOpenDownwardsClass: 'js-swiped--will-open-downwards',
      willOpenUpwardsClass: 'js-swiped--will-open-upwards',
    };
    ...
    this.time = o.time;
    this.animDistance = o.animDistanceDown || o.animDistanceUp; // done
    this.elem = o.elem;
    this.list = o.list;
    this.dir = o.dir;
    this.group = o.group;
    ...
    this.willOpenDownwardsClass = o.willOpenDownwardsClass;
    this.willOpenUpwardsClass = o.willOpenUpwardsClass;

    this.animDistanceUp = o.animDistanceUp;
    this.animDistanceDown = o.animDistanceDown;

    if (
      (o.animDistanceUp > 0 && o.threshold > o.animDistanceUp) ||
      (o.animDistanceDown > 0 && o.threshold > o.animDistanceDown)
    ) {
      console.warn('threshold must be less then animDistanceDown and animDistanceUp');
    }
  };
  ...
  Swiped.prototype.touchMove = function (e) {
    ...
    this.animDistance = this.delta < 0 ? this.animDistanceUp : this.animDistanceDown; // done
  ...
  Swiped.prototype.touchEnd = function (e) {
    ...
    // if swipe is more then threshold or time is less then 150ms
    // check animDistance to avoid opening when animDistance we can only swipe up, and finger moves down past threshold
    if (
      this.animDistance > 0 &&
      (this.dir * this.delta > this.threshold ||
        (!this.retracted && new Date() - this.startTime < this.time))
    ) {
      this.open();
    } else {
      this.close();
  ...
  Swiped.prototype.open = function (isForce) {
    this.animation(this.dir * this.animDistance);
    this.swiped = true;
  ...
  Swiped.prototype.close = function (isForce) {
    ...
    this.elem.classList.remove(
      this.willOpenClass,
      this.willOpenDownwardsClass,
      this.willOpenUpwardsClass,
    );
  ...
  Swiped.prototype.move = function () {
    if (
      (this.dir > 0 && (this.delta < 0 || this.animDistanceDown === 0)) ||
      (this.dir < 0 && (this.delta > 0 || this.animDistanceUp === 0))
    ) {
      return false;
    }
    ...
    if (this.dir * this.delta > this.threshold) {
      if (!this.willOpen) {
        this.willOpen = true;
        let willOpenDirectionClass = this.willOpenUpwardsClass;
        if (this.dir === 1) {
          willOpenDirectionClass = this.willOpenDownwardsClass;
        }
